...
> Fork note: this project is forked from [Fluent-networks/tailscale-mikrotik](https://github.com/Fluent-networks/tailscale-mikrotik).
...
| ----------------- | --------------------------------------------- | -------------------------------------------- |
| PASSWORD          | System root user password                     | Optional. Set only if you want SSH password login to the container |
| AUTH_KEY          | Tailscale non-reusable key, [Oauth secret](https://tailscale.com/kb/1215/oauth-clients#registering-new-nodes-using-oauth-credentials) or Headscale pre-authenticated key                       | Optional. If not set, login is interactive (non-headless) from container shell         |
| ADVERTISE_ROUTES  | Comma-separated list of routes to advertise   | Optional. Required only for subnet-router use cases |
| CONTAINER_GATEWAY | The container bridge (veth1) IP address on the router | Optional unless `ADVERTISE_ROUTES` is set |
...
Example interactive Tailscale login (no `AUTH_KEY`, Linux-server style):
```
/container/envs
add list="tailscale" key="PASSWORD" value="xxxxxxxxxxxxxx"
add list="tailscale" key="ADVERTISE_ROUTES" value="192.168.88.0/24"
add list="tailscale" key="CONTAINER_GATEWAY" value="172.17.0.1"
add list="tailscale" key="TAILSCALE_ARGS" value="--accept-routes"
```
After the container starts, open shell and run:
```
/container/shell 0
...
If you did not configure `AUTH_KEY`, complete interactive login first:
...
The command prints a login URL. Open it in your browser, approve the node, then verify:
